				...
				<form action="{{ route('update_thesis',$thesis->id) }}" method="POST" enctype="multipart/form-data">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
						<div class="grid grid-cols-1">
							@php
							$year = $thesis->awarded_at;
							@endphp
							<label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">{{__('publish_year')}}</label>
							<input class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="text"  value="{{$year}}" placeholder="yyyy" required name="year"/>
						</div>
