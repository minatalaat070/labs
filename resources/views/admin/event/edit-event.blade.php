		...
		<div class="flex h-auto bg-gray-200 items-center justify-center p-10">
			<div class="grid bg-white rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-1/2">
				...
				<div class="flex justify-center">
					<div class="flex">
						<h1 class="text-gray-600 font-bold md:text-2xl text-xl">{{__('edit_event')}}</h1>
					</div>
				</div>
				<form action="{{route("update_event",$event->id)}}" method="POST" enctype="multipart/form-data">
					...
					<div class="grid grid-cols-1 mt-5 mx-7">
						...
						<input class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="text" name="name" value="{{$event->name}}"/>
					</div>
					<div class="grid grid-cols-1 mt-5 mx-7">
					...
					<div class="grid grid-cols-1 mt-4 mx-7">
						...
						<select  name="lab_id" class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent">
							<option value="{{$event->lab->id}}">{{$event->lab->name}}</option>
							@foreach($labs as $lab)
							@if(!($lab->id == $event->lab->id))
