		...
		<div class="flex h-auto bg-gray-200 items-center justify-center p-10">
			<div class="grid bg-white rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-1/2 mt-16">
				...
				<div class="flex justify-center">
					<div class="flex">
						<h1 class="text-gray-600 font-bold md:text-2xl text-xl">{{__('edit_member')}}</h1>
					</div>
				</div>
				<form action="{{route("update_member",$member->id)}}" method="POST" enctype="multipart/form-data">
					...
					<div class="grid grid-cols-1 mt-4 mx-7">
						...
						<input class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="text" name="name" value="{{$member->name}}"/>
					</div>
					<div class="grid grid-cols-1 mt-4 mx-7">
						<label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">{{__('name_in_arabic')}}</label>
						<input dir="rtl" class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="text"  name="name_ar" value="{{$member->name_ar}}"/>
					</div>
					<div class="grid grid-cols-1 mt-5 mx-7">
						<label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">{{__('staff_url')}}</label>
						<input class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="url" name="staff_url" value="{{$member->staff_url}}"/>
					</div>
					<div class="grid grid-cols-1 mt-4 mx-7">
						...
						<select  name="lab_id" class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent">
							<option value="{{$member->lab->id}}">{{$member->lab->name}}</option>
							@foreach($labs as $lab)
							@if(!($lab->id == $member->lab->id))
					...
					<!-- TODO  handle cv upload process and add input field for home page-->
					<div class="grid grid-cols-1 mt-5 mx-7">
						<label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold mb-1">{{__('upload_cv')}}</label>
						<div class='flex items-center justify-center w-full'>
							<label class='flex flex-col border-4 border-dashed w-full h-32 hover:bg-gray-100 hover:border-purple-300 group'>
								<div class='flex flex-col items-center justify-center pt-7'>
									<svg class="w-10 h-10 text-purple-400 group-hover:text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
									...
									<p class='lowercase text-sm text-gray-400 group-hover:text-purple-600 pt-1 tracking-wider'>{{__("select_cv")}}</p>
								</div>
								<input name="cv_file" type='file' accept=".pdf,.doc,.docx" class="hidden" />
							</label>
						</div>
					</div>
